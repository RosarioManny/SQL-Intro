...
-- Carmen Sandiego - Name ; Southern Europe - Country ; least population - Population
-- A - Vatican City - Population of 1000; 
...
-- Look at countrylanguages table referencing the Country Code and look at the language column. Vatican City is VAT. L
-- A - Italian
...
-- Look for a country where population speaks only Italian(100%).
-- A - San Marino
...
-- Write SQL query here
SELECT * FROM cities JOIN countries ON countrycode = 'SMR' AND code = 'SMR' AND cities.name != 'San Marino';
-- Find the city that is not the same name as the country
-- A - Serravalle
...
-- Write SQL query here
-- SELECT name FROM countries WHERE countries.region = 'South America' AND countries.code LIKE 'S%';
-- SELECT * FROM cities WHERE name ILIKE 'San%';

SELECT 
   cities.name, cities.countrycode
FROM
    cities
JOIN
    countries
...
WHERE 
    cities.name ILIKE 'Ser%' AND countries.region = 'South America';
-- A - Serra, Brazil
...
ON 
   cities.countrycode = countries.code
WHERE 
    cities.id = countries.capital AND countries.region = 'South America' AND countries.code = 'BRA';

-- A -  Brasília | Brazil
-- Countries has capital stores as INT; cities has to equal the coutries capital number.

-- Clue #7: 
-- She knows we're on to her – her taxi dropped her off at the international airport, and she beat us to the boarding gates.
...
-- city population  >= 91000 + 1 AND capital = 85

SELECT
    cities.name AS city, countries.name AS country, cities.population, cities.id
FROM
    cities
JOIN
    countries
